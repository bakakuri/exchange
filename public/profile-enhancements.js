(() => {
  ...
  style.textContent = `
    ...
    .profile-edit-form{overflow:hidden;transition:max-height .25s ease,opacity .2s ease}.profile-edit-form.is-closed{max-height:0!important;opacity:0}.profile-edit-form.is-open{max-height:1000px;opacity:1}.profile-auth-hidden{display:none!important}
    @media(max-width:800px){.profile-account-strip{align-items:flex-start;flex-direction:column}}
  `;
  document.head.appendChild(style);

  // Fix the original preview null-reference before app.js (defer) executes.
  ...
  function authVisible() {
    // app.js exposes the real state only inside its closure, so use its auth UI as a bridge.
    const btn=q('#authBtn');
  ...
  function applyAuthGate(){
    const logged=authVisible();
    qa('[data-view="profile"],#profileNav').forEach(el=>el.classList.toggle('profile-auth-hidden',!logged));
    const view=q('#profile-settings');
    if(view) view.classList.toggle('profile-auth-hidden',!logged);
  }
  ...
  function setupEditor(){
    ...
    if(!row){row=document.createElement('div');row.className='profile-edit-row';row.innerHTML='<p>ანგარიშის ძირითადი ინფორმაციის შეცვლა</p><button type="button" class="profile-edit-toggle" aria-expanded="false">✎ რედაქტირება</button>';card.insertBefore(row,form);}
  ...
  function placeIdentity(){
    ...
    const email=q('#settingsEmail')?.textContent?.trim()||'';
    const role=q('#userPlan')?.textContent?.trim()||'მომხმარებელი';
    ...
    if(next&&!next.classList.contains('profile-account-strip'))next.replaceWith(strip); else if(strip.parentElement!==card)avatar.insertAdjacentElement('afterend',strip);
  }

  function render(){
    applyAuthGate(); if(!authVisible())return;
    setupEditor();placeIdentity();
    ...
    const tx=qa('#transactionsList .transaction').slice(0,5);
