-- Exchange: final complete_task guardrail fix.
-- Applies the intended Starter-task exception and verification gate.
...
begin
  ...
  select owner_id,reward,category,promotion_id,target_url,platform,action
  into v_owner,v_reward,v_category,v_promotion,v_target_url,v_platform,v_action
  from public.tasks
  ...
  if v_owner is not distinct from v_user
     and coalesce(v_category,'') <> 'Starter' then
    raise exception 'You cannot complete your own task';
  end if;

  if not public.exchange_valid_target_url(v_platform,v_target_url) then
  ...
  insert into public.credit_transactions(
    ...
    case when v_category='Starter' then 'Starter task reward' else 'Approved task verification reward' end,
    v_balance+v_reward
  );
  ...
  perform public.write_audit_log(
    ...
    jsonb_build_object('promotion_id',v_promotion,'platform',v_platform,'action',v_action)
  );
