...
begin
  ...
  if v_count=1 then
    if not exists(select 1 from pg_policies where schemaname='public' and tablename='notifications' and policyname='notifications_insert_own' and with_check='false') then
      raise exception 'TEST FAILED: notification client insert policy is not locked';
    end if;
  end if;
